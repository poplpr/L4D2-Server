...
public void OnPluginStart()
{
	...
	alonemode = CreateConVar("alonemode", "0", "Is alonemode or not");
...
{
	if(alonemode.IntValue == 0)
		SetConVarInt(convar, 1); // hack-hack: game tries to change this cvar for some reason, can't be arsed so HARDCODETHATSHIT
	else
		SetConVarInt(convar, 10); // hack-hack: game tries to change this cvar for some reason, can't be arsed so HARDCODETHATSHIT
}
